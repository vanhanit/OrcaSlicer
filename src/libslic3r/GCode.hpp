...
private:
    ...
    // Orca: the labels bracketing one instance's extrusions within a layer - the gcode_label_objects
    // comment and the firmware-specific exclude-object markers. Shared by every site that starts or
    // finishes printing an instance.
    std::string     object_start_labels(const Print& print, const InstanceToPrint& instance_to_print, const PrintInstance& inst);
    void            object_end_labels(const Print& print, const InstanceToPrint& instance_to_print, const PrintInstance& inst);
